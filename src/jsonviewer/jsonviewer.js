!function($){
	...
	$.fn.jsonviewer = function(jjson, first) {
    return this.each( function() {
    	...
    	new JJsonViewer(self, first);
    });
	};

	function JJsonViewer(self, first) {
		self.html('<ul class="jjson-container"></ul>');
		try {
			var json = $.parseJSON(self.data('jjson'));
			self.find('.jjson-container').append(json2html([json], first));
		} catch(e) {
	...
	function json2html(json, first) {
		...
		for(var key in json) {
			...
			html = html + createElement(key, value, type, getExpanderClasses(first), first);
		}
		return html;
	}
	...
	function createElement(key, value, type, expanderClasses) {
		...
		if(type == 'object') {
			...
			object = object + json2html(value, false);
